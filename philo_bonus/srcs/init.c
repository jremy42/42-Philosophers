...
/*   Updated: 2022/03/03 12:32:55 by jremy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philosophers_bonus.h"

int	__check_data(t_global *global, int ac)
{
	if (global->number_of_philo <= 0 || global->number_of_philo > 200)
		return (__putstr_fd("wrong number of philo\n", 2), 0);
	if (global->time_to_die <= 0)
		return (__putstr_fd("Time to die is negative number or zero\n", 2), 0);
	if (global->time_to_eat <= 0)
		return (__putstr_fd("Time to eat is negative number or zero\n", 2), 0);
	if (global->time_to_sleep <= 0)
		return (__putstr_fd("Time to sleep is negative number or zero\n", 2), 0);
	if (ac == 6)
		if (global->max_eat <= 0)
			return (__putstr_fd("max est is negative number or zero\n", 2), 0);
	return (1);
}
